...
export default function ReservationDetailPage() {
  ...
  const handleUpdateSubmit = async (updatedData: ReservationItemType) => {
    ...
    try {
      ...
      alert(`예약 수정 실패! ${err.response?.data?.message || err.message}`);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      alert(`예약 삭제 실패! ${err.response?.data?.message || err.message}`);
    }
  };
      ...
      <PageWrap>
        {!editing ? ( // 수정 모드가 아닐 때 (기존 상세 보기 화면)
          <>
            ...
            {/* 예약 내용 표시 */}
            {reservation.contentType === "text" && <p>{reservation.content}</p>}
            {reservation.contentType === "table" && reservation.contentData && (
              <Table>
                <thead>
                  <tr>
                    {reservation.contentData.headers?.map((h, i) => (
                      <th key={i}>{h}</th>
                    ))}
                  </tr>
                </thead>
